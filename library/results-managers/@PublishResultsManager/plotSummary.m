...
for key = keys(results)
    ...
    for i = 1:length(keyRes)
        ...
        percInHypo(i) = sum(yvalAll(i, :) < 3.9) / size(yvalAll, 2);
        meanGlucose(i) = mean(yvalAll(i, :));
        sdGlucose(i) = std(yvalAll(i, :));
    ...
    annotation('textbox', [0.77, 0.4, 0, 0], ...
        'FitBoxToText', 'on', ...
        'string', sprintf( ...
        'In Target : %04.2f (%04.2f) %%\nIn Hypo : %04.2f[%04.2f - %04.2f] %%\n Mean Glucose : %04.2f (%04.2f) (mmol/L)\n SD Glucose : %04.2f[%04.2f - %04.2f] (mmol/L)\n Count Hypo : %04.2f[%04.2f - %04.2f]', ...
        round(mean(percInRange)*100, 2), ...
        round(std(percInRange)*100, 2), ...
        round(prctile(percInHypo, [50, 25, 75])*100, 2), ...
        round(mean(meanGlucose), 2), ...
        round(std(meanGlucose), 2), ...
        round(prctile(sdGlucose, [50, 25, 75]), 2), ...
